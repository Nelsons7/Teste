...
function printPilha(item, index){
    ...
    node.className = "btn btn-outline-primary btn-block";
...
function verificaUltimo(){
    ...
    document.getElementById("avisos").innerHTML = " O ultimo elemento é: " + pilha.peek(); 
        ...
        alert("A pilha está vazia"); return; 
        
    }
}
function removeUltimo(nomeBotao) {
    
    var ultimoValor = pilha.peek();
    if(ultimoValor == nomeBotao){
        pilha.pop();//1
        pilha.print();//2
        document.getElementById("avisos").innerHTML = "<div class='alert alert-success'>Elemento removido</div>";
    } else {
        document.getElementById("avisos").innerHTML = "<div class='alert alert-danger'>Este não é o ultimo elemento! Não é Possivel Remove-lo!</div>";
    }
